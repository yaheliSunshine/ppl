...
import { unbox, setBox, Box} from "../shared/box";
...
bind(makeOk(unbox(bdg.val)), (v) => isClosure(v)? 
    makeOk(setFBinding(bdg, makeTracedClosure(v, bdg.var))) :
    makeFailure("not a procedure")));
...
const applyTracedClosure = (proc: TracedClosure, args: Value[]): Result<Value> => {
    ...
    setBox(proc.counter, unbox(proc.counter)-1);
    mapv(val, (x) => printPostTrace(x,unbox(proc.counter)));
    return val;
}
