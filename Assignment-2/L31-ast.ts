...
const parseLetPlusExp = (bindings: Sexp, body: Sexp[]): Result<LetPlusExp> => {
    ...
    // After isGoodBindings, bindings has type [string, Sexp][]
    //const test = mapResult(bind(b => makeBinding(b[0], parseL31CExp(b[1]))), bindings);

    const bindingsResult = mapResult((vals: [string, Sexp]) : Result<Binding> => bind(parseL31CExp(vals[1]), (val : CExp) => makeOk(makeBinding(vals[0], val))), bindings)
